...
function ExistingUserStep({ onBack, onSuccess }: { onBack: () => void; onSuccess: () => void }) {
  ...
  const handleSearch = async (q: string) => {
    ...
    try {
      ...
      const data = (await res.json()) as { startups?: StartupResult[] };
...
// ─── Step: New user (email invite) ───────────────────────────────────────────

function NewUserStep({ onBack, onSuccess }: { onBack: () => void; onSuccess: () => void }) {
  ...
  const handleSubmit = async (e: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!res.ok) { setError(json.error ?? "Failed to send invite. Please try again."); setBusy(false); return; }
      setSent(true);
  ...
  return (
    <form onSubmit={(e) => void handleSubmit(e)} className="space-y-4">
      ...
      <div>
        <label htmlFor="invite-message" className="block text-xs font-semibold text-[#F4F4FF]">
