...
> = {
  match:    { dot: "bg-indigo-500",  icon: "★", bg: "bg-indigo-50"  },
  accepted: { dot: "bg-emerald-500", icon: "✓", bg: "bg-emerald-50" },
  intro:    { dot: "bg-purple-500",  icon: "↔", bg: "bg-purple-50"  },
  declined: { dot: "bg-gray-300",    icon: "✕", bg: "bg-gray-50"    },
};
    ...
    <div className="min-h-screen bg-(--color-canvas)">
      <section className="border-b border-(--color-hairline) bg-(--color-surface-soft) px-[5%] py-10">
        <div className="mx-auto max-w-3xl">
          ...
          <h1 className="mt-3 text-3xl font-semibold text-(--color-ink)">Notifications</h1>
          <p className="mt-2 text-sm text-(--color-body)">Your matches, invites, and platform activity.</p>
        </div>
      </section>
      ...
      <div className="mx-auto max-w-3xl px-[5%] py-10">
        {loading ? (
          ...
          <div className="rounded-2xl border border-(--color-hairline) bg-(--color-surface-soft) px-8 py-16 text-center">
            ...
            <p className="mt-4 text-base font-semibold text-(--color-ink)">No notifications yet</p>
                ...
                <div className="space-y-2">
                  {items.map((n) => {
                    ...
                    return (
                      ...
                      >
                        <span className={`mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-full ${bg}`}>
                          <span className={`h-4 w-4 rounded-full ${dot} flex items-center justify-center text-[9px] font-bold text-white`}>
                            {icon}
                          </span>
                        </span>
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="mt-0.5 text-sm text-(--color-body)">{n.body}</p>
                          <p className="mt-1 text-xs text-(--color-muted)">{formatRelativeTime(n.date)}</p>
                        </div>
                        <span className="shrink-0 text-xs text-(--color-muted)">→</span>
                      </Link>
                    );
                  })}
                </div>
